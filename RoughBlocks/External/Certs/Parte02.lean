...
noncomputable def LB_CORE (m x : ℕ) : ℝ :=
  ((m : ℝ) / Real.log (m : ℝ)) *
...
noncomputable def margin_CORE (m : ℕ) : ℝ :=
...
/-- `log m > 0` para `m ≥ 2`. -/
lemma log_pos_of_ge_two_CORE {m : ℕ} (hm : 2 ≤ m) : 0 < Real.log (m : ℝ) := by
  have hm1 : (1 : ℝ) < (m : ℝ) :=
...
lemma u_block_range_of_ge_U0_CORE
...
∧ Real.log ((m^2 + x*m : ℕ) : ℝ) / Real.log (m : ℝ) ≤ (3 : ℝ) := by
  ...
  have hlogpos : 0 < Real.log (m : ℝ) := log_pos_of_ge_two_CORE hm2
...
lemma LB_ge_margin_CORE' {m x : ℕ} (hm : U0Default ≤ m) (hx : x ≤ 8) :
  LB_CORE m x ≥ margin_CORE m := by
  unfold RoughBlocks.Heavy.LB_CORE RoughBlocks.Heavy.margin_CORE
  have hlogpos : 0 < Real.log (m : ℝ) :=
    log_pos_of_ge_two_CORE (le_trans (by decide : (2:ℕ) ≤ U0Default) hm)
  have hcoef_nonneg : 0 ≤ (m : ℝ) / Real.log (m : ℝ) :=
    div_nonneg (by positivity) hlogpos.le
  have hRange := u_block_range_of_ge_U0_CORE (m := m) (x := x) hm hx
  have hω : (1 : ℝ) / 3 ≤
      omega (Real.log ((m^2 + x*m : ℕ) : ℝ) / Real.log (m : ℝ)) :=
