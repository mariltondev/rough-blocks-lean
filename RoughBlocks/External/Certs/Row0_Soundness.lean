...
/-- Hipótese natural na camada `Φ`: bound uniforme
    `PhiDiffAt m 0 ≥ row0.finalLowerLo` para `m ≥ 10^6`. -/
class Row0PhiDiffBound1e6 : Prop where
  bound :
    ∀ {m : ℕ}, 1_000_000 ≤ m → PhiDiffAt m 0 ≥ (row0.finalLowerLo : ℝ)

/-- A hipótese em `Φ` implica diretamente a hipótese em `count`
    usando a ponte já implementada em `UniformGE1e6Bridge`. -/
instance (priority := 100) Row0PhiDiff_to_WindowBound
    [Row0PhiDiffBound1e6] : Row0WindowLinkBound1e6 :=
by
  refine { bound := ?_ }
  intro m hm
  have h :=
    row0_count_ge_finalLowerLo_1e6_of_phiDiff_bound
      (m := m)
      (hΦ := Row0PhiDiffBound1e6.bound)
      hm
  simpa using h

/-- A própria instância em `Φ` já entrega o certificado por-linha. -/
def row0_certificate_from_phiDiff
    [Row0PhiDiffBound1e6] : Row0Certificate :=
{ finalLo_is_q := rfl
  count_ge_q := by
    intro m hm
    have h :=
...
#print axioms RoughBlocks.External.Certs.count_row0_ge_one_1e6_noHyp
#check RoughBlocks.External.Certs.count_row0_ge_one_1e6_noHyp
