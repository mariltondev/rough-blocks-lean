...
-- Se você ainda não tem, mantenha estes como lemmas/axiomas mínimos.
-- `Ay_const_on_block`: A_y é constante em cada bloco unitário.
lemma Ay_const_on_block
  (y X n : ℕ) :
  ∀ (t : ℝ), t ∈ Ι (X+n : ℝ) (X+n+1 : ℝ) →
    A_y y t = A_y y (X+n : ℝ) := by
  sorry

-- `FTC_block`: versão do TFC para g' no bloco unitário.
lemma FTC_block (X n : ℕ) :
  ∫ t in (X+n : ℝ)..(X+n+1 : ℝ), g' t
    = g (X+n+1 : ℝ) - g (X+n : ℝ) := by
  sorry

-- Divide a integral grande em blocos unitários (usa o lema com nome correto).
lemma integral_split_into_unit_blocks :
  ∀ (X Y : ℕ) (f : ℝ → ℝ),
    ∫ t in (X : ℝ)..(X+Y : ℝ), f t
      = (Finset.range Y).sum (fun n => ∫ t in (X+n : ℝ)..(X+n+1 : ℝ), f t)
| X, 0, f => by
  simp
| X, Nat.succ Y, f => by
  have IH := integral_split_into_unit_blocks X Y f
  -- use the adjacent-additivity of the integral (fully qualified) to avoid unresolved side-goals
  have hstep :
      ∫ t in (X : ℝ)..(X+Y+1 : ℝ), f t
        = ∫ t in (X : ℝ)..(X+Y : ℝ), f t
          + ∫ t in (X+Y : ℝ)..(X+Y+1 : ℝ), f t := by
    simpa using intervalIntegral.integral_add_adjacent (a := (X : ℝ)) (b := (X+Y : ℝ)) (c := (X+Y+1 : ℝ)) (f := f)
  calc
    ∫ t in (X : ℝ)..(X+Y+1 : ℝ), f t
      = ∫ t in (X : ℝ)..(X+Y : ℝ), f t + ∫ t in (X+Y : ℝ)..(X+Y+1 : ℝ), f t := hstep
    _ = (Finset.range Y).sum (fun n => ∫ t in (X + n : ℝ)..(X + n + 1 : ℝ), f t) + ∫ t in (X+Y : ℝ)..(X+Y+1 : ℝ), f t := by
      rw [IH]
    _ = (Finset.insert Y (Finset.range Y)).sum (fun n => ∫ t in (X + n : ℝ)..(X + n + 1 : ℝ), f t) := by
      have hnot : Y ∉ Finset.range Y := by simp [Finset.not_mem_range_self]
      rw [Finset.range_succ, Finset.sum_insert hnot]
      simp
    _ = (Finset.range (Y+1)).sum (fun n => ∫ t in (X + n : ℝ)..(X + n + 1 : ℝ), f t) := by
...
lemma abel_block_partition_neg
  ...
  classical
  have hsplit :=
    integral_split_into_unit_blocks X Y (fun t => g' t * A_y y t)

  -- integral em cada bloco: puxe a constante A_y y (X+n) e aplique TFC
  have hblock :
      ∀ n ∈ Finset.range Y,
        ∫ t in (X+n : ℝ)..(X+n+1 : ℝ), g' t * A_y y t
          = (A_y y (X+n : ℝ)) * (g (X+n + 1 : ℝ) - g (X+n : ℝ)) := by
    intro n hn
    have hle : (X+n : ℝ) ≤ (X+n+1 : ℝ) := by
      norm_cast; exact by exact Nat.le_succ _
    have hconst_on_Icc :
      ∀ t ∈ Ι (X+n : ℝ) (X+n+1 : ℝ),
        A_y y t = A_y y (X+n : ℝ) :=
      Ay_const_on_block y X n

    -- iguale o integrando a.e. em Ι(a,b)
    have hAE :
      ∀ᵐ x, x ∈ Ι (X+n : ℝ) (X+n+1 : ℝ) →
        g' x * A_y y x = A_y y (X+n : ℝ) * g' x := by
      refine Filter.Eventually.of_forall ?_
      intro x hxΙ
      have hxconst := hconst_on_Icc x hxΙ
      simp [hxconst, mul_comm]
    ...
    calc
      ...
      _ = (A_y y (X+n : ℝ)) *
            ∫ t in (X+n : ℝ)..(X+n+1 : ℝ), g' t := by
            simpa using
              intervalIntegral.const_mul
                (c := A_y y (X+n : ℝ))
                (f := g')
      _ = (A_y y (X+n : ℝ)) * (g (X+n + 1 : ℝ) - g (X+n : ℝ)) := by
            simp [hFTC]

  calc
    ∫ t in (X : ℝ)..(X+Y : ℝ), g' t * A_y y t
        = (Finset.range Y).sum
            (fun n => ∫ t in (X+n : ℝ)..(X+n+1 : ℝ), g' t * A_y y t) := by
          simpa using hsplit
    _ = (Finset.range Y).sum
          (fun n =>
            (A_y y (X+n : ℝ)) * (g (X+n + 1 : ℝ) - g (X+n : ℝ))) := by
          refine Finset.sum_congr rfl ?_
          intro n hn; simpa using hblock n hn
...
-- -- (2)
-- lemma sum_range_by_parts_shifted'
--   (Y : ℕ) (u v : ℕ → ℝ) :
--   Finset.sum (Finset.range Y) (fun n => u n * (v (n + 1) - v n))
--     =
--     u Y * v Y - u 0 * v 0
--     - Finset.sum (Finset.range Y) (fun n => v (n + 1) * (u (n + 1) - u n)) := by
--   sorry

-- -- (3)
-- lemma bridge_phiDiff_as_primeJumpSum
--   (X Y y : ℕ) (hX : 1 ≤ X) (hY : 1 ≤ Y) (hy : 2 ≤ y) :
--   Finset.sum (Finset.range Y)
--     (fun n =>
...
--     - (RoughBlocks.Heavy.PhiGE X y : ℝ)) := by
--   sorry
