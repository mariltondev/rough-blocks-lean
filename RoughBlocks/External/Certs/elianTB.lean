...
theorem one_mRough_in_allNine_from_18794_ipth
  ...
  #check RoughBlocks.External.Certs.one_mRough_in_allNine_from_18794_ipth
...
theorem exists_mRough_in_allNine_from_18794_hpt
  ...
  -- #check RoughBlocks.External.Certs.exists_mRough_in_allNine_from_18794_hpt
...
-- @[simp] lemma cert_full_true :
--   RoughBlocks.External.Certs.FullVerifier.verify_full_certificate = true := by
--   decide

-- /-- ≥1 m-áspero em cada bloco, m ≥ 18794 (sem axiomas). -/
-- theorem exists_mRough_in_allNine_from_18794
--   {m : ℕ} (hm : 18794 ≤ m) :
--   ∀ x : Fin 9, ∃ k ∈ K m (x : ℕ), mRough m k := by
--   intro x
--   -- dos seus lemas já verificados pelo certificado: ≥ 2 por bloco
--   have hx : (x : ℕ) ≤ 8 := Nat.le_of_lt_succ x.2
--   have h2 := two_mRough_from_uniformLB cert_full_true hm hx
--   -- “≥ 2” ⇒ “existe”
--   have hpos : 0 < ((K m (x : ℕ)).filter (mRough m)).card :=
--     Nat.lt_of_lt_of_le (by decide : 0 < 1) ((by decide : 1 ≤ 2).trans h2)
...
--   intro x
--   have hx : (x : ℕ) ≤ 8 := Nat.le_of_lt_succ x.2
--   have h2 := two_mRough_from_uniformLB RoughBlocks.External.Certs.cert_full_true hm hx
--   exact (le_trans (by decide : 1 ≤ 2) h2)
...
--   ∀ x : Fin 9, ∃ k ∈ K m (x : ℕ), mRough m k := by
--   intro x
--   have h1 := one_mRough_in_allNine_from_18794 (m := m) hm x
--   have hpos : 0 < ((K m (x : ℕ)).filter (mRough m)).card :=
--     Nat.lt_of_lt_of_le (by decide : 0 < 1) h1
--   obtain ⟨k, hk⟩ := Finset.card_pos.mp hpos
--   rcases Finset.mem_filter.mp hk with ⟨hkK, hkR⟩
