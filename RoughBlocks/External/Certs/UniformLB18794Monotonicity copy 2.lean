...
/-- Versão final “∀ x, card≥1” para `m ≥ 18794`, **parametrizada** pelo adaptador `H`. -/
theorem one_mRough_in_allNine_ge_18794_using_H
  (H : UniformBridgeFrom_m0) {m : ℕ} (hm : (18794 : ℕ) ≤ m)
  (h_m0_eq_18794 : m0 = 18794 := rfl) :
  ∀ x : Fin 9, 1 ≤ ((K m (x : ℕ)).filter (mRough m)).card :=
by
  have hm' : m0 ≤ m := by simpa [h_m0_eq_18794] using hm
  exact one_mRough_in_allNine_from_18794_using_uniformBridge H (m := m) hm'

theorem exists_allNine_ge_18794_using_H
  (H : UniformBridgeFrom_m0) {m : ℕ} (hm : (18794 : ℕ) ≤ m)
  (h_m0_eq_18794 : m0 = 18794 := rfl) (x : Fin 9) :
  ∃ k ∈ K m (x : ℕ), mRough m k := by
  -- Converte 18794 ≤ m em m0 ≤ m usando m0 = 18794
  have hm' : m0 ≤ m := by simpa [h_m0_eq_18794] using hm
  -- x ≤ 8
  have hx : (x : ℕ) ≤ 8 := Nat.le_of_lt_succ x.2
  -- Aplica a versão para m ≥ m0 já provada
  exact exists_mRough_in_block_ge_m0_using_uniformBridge H (m := m) (x := x) hm' hx
