...
/-- lower bound conservador e uniforme para ω(u).
    (0.49 é < a todos os ω_lo dos 9 blocos do seu JSON, então é seguro.) -/
def omegaLo_q (_x : Fin 9) : ℚ := 49 / 100

/-- vértice da parábola simplificada f(L) = ω(L - v)^2 + const, em ℝ. -/
def vL (x : Fin 9) : ℝ :=
...
/-- bound numérico grosseiro: com ω_lo = 0.49 e C1 = 4.4, vL < 5. -/
