...
lemma omega_ge_one_half_on_Icc_two_three
  ...
  have hg_int :
    ...
    have hconst :
      ...
      continuousOn_const
    exact hconst.intervalIntegrable
  ...
  have hlog_ge : (t - 1)/2 ≤ Real.log t := by
    have : 0 ≤ Real.log t - (t - 1)/2 := by
      rwa [hint_value] at hint_nonneg
    exact sub_nonneg.mp this

  -- então u/2 ≤ 1 + log t
  have hnum : u/2 ≤ 1 + Real.log t := by
    -- add 1 to both sides of hlog_ge: 1 + (t - 1)/2 ≤ 1 + log t
    have h_add := add_le_add_left hlog_ge (1 : ℝ)
    -- rewrite 1 + (t - 1)/2 as (t + 1)/2 and use the previous inequality
    have h_mid : (t + 1)/2 ≤ 1 + Real.log t := by
      have : (t + 1)/2 = 1 + (t - 1)/2 := by ring
      rw [← this] at h_add
      exact h_add
    -- use t = u - 1 to finish: (t+1)/2 = u/2
    simpa [t] using h_mid
