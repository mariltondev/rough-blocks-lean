# external/julia/bridge_increment_1e6.jl
#
# Certificado "forte" (esqueleto) para a ponte incremental: m ≥ 1_000_000, x ∈ {0..8}
# Saída: certs/uniform-bridge-increment-1e6.json
#
# julia --project=. external/julia/bridge_increment_1e6.jl
#
...
# conservadoras (placeholders) negativas/zero; troque-os pelas cotas refinadas por x
# para obter delta_lo_q ≥ 0 e (quando você usar o conversor p/ Lean) eliminar qualquer “admit”.
...
function terms_for_x(x::Int; pbits::Int=PBITS)
    setprecision(pbits) do
        ...
        # Variação (M2) e resíduo (R) agregados em C1 (placeholders)
        m2_lo = BigFloat(0)
        r_lo  = BigFloat(0)
...
function build_payload(; pbits::Int=PBITS)
    ...
    meta = Dict(
        ...
        "notes"  => "Strong certificate with rational witnesses for m >= 1e6. Replace m2_lo_q and r_lo_q by refined per-x lower bounds to push delta_lo_q ≥ 0 quando necessário."
    )
...
function main(outpath::String="certs/uniform-bridge-increment-1e6.json")
